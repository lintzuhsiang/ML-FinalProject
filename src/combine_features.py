...
# Usage: combine_features.py ../data/ ../workspace/train_combined_features.csv
...
train_data = pd.read_csv( args.dataDirectory + 'clicks_train.csv', delimiter=',', \
													dtype={'display_id':np.int32, 'ad_id':np.int32, 'clicked':bool})


""" concatenate events features into train_data """
# read events data
if( args.printInfo ):
...
in_array = np.in1d( train_data[:]['display_id'], events_data[:]['display_id'] )
idx_found = np.searchsorted( events_data[:]['display_id'], train_data[:]['display_id'] )
...
train_data = pd.concat( (train_data, to_append), axis=1 )


""" concatenate documents_meta features into train_data (display documents) """
...
in_array = np.in1d( train_data[:]['display_document_id'], documents_meta_data[:]['document_id'] )
idx_found = np.searchsorted( documents_meta_data[:]['document_id'], train_data[:]['display_document_id'] )
...
""" concatenate promoted_content features into train_data """
if( args.printInfo ):
...
in_array = np.in1d( train_data[:]['ad_id'], promoted_content_data[:]['ad_id'] )
idx_found = np.searchsorted( promoted_content_data[:]['ad_id'], train_data[:]['ad_id'] )
...
""" concatenate documents_meta features into train_data (ad documents) """
if( args.printInfo ):
	print "Extract_features.py: processing input data - documents_meta.csv (for ad documents)"	

# search for the corresponding row in events_data by display_id
# documents_meta_data.sort_values( ['document_id'], inplace=True )
in_array = np.in1d( train_data[:]['ad_document_id'], documents_meta_data[:]['document_id'] )
idx_found = np.searchsorted( documents_meta_data[:]['document_id'], train_data[:]['ad_document_id'] )
idx_in_limit = ( idx_found != len(documents_meta_data) )
idx_found = idx_found * np.logical_and(idx_in_limit, in_array) + (-1)*( np.logical_not(np.logical_and(idx_in_limit, in_array)) )
idx_found_bool = np.concatenate(( ( idx_found > -1 ).reshape(-1,1), ( idx_found > -1 ).reshape(-1,1) ), axis=1)
to_append = documents_meta_data.iloc[idx_found, 1:3] * (idx_found_bool) + np.array([-1,-1])*(1-(idx_found_bool))
...
train_data.to_csv( args.outputFile, sep=',' )

train_data_reduced = train_data.iloc[:, [3,6,7,8,9,11,12,13,14]]
train_data_reduced.to_csv( 'reduced_' + args.outputFile+, sep=',' )